...
//       then calls a function to perfrom the requested
//       action. Repeats until the user enters the
//       command 'q'. Prints an error message if the
...
void changeprice(void) {
  ...
  printf("Enter new price for part %d: ", i);
  scanf("%f", &inventory[i].price);
  printf("New price for part %d: %.2f\n", inventory[i].number,
         inventory[i].price);
}
...
void insert(void) {
  ...
  printf("Enter price: ");
  scanf("%g", &inventory[num_parts].price);
  num_parts++;
}

// search: Prompts the user to enter a part number,
//         then looks up the part in the database.
//         If the part exsits, prints the name and
//         and quantity on hand; if not, prints
//         an error message.
